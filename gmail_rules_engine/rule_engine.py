...
class RuleEngine:
    ...
    def _evaluate_rule(self, email: Email, rule: Dict[str, Any]) -> bool:
        ...
        if predicate == "all":
            return all(self._evaluate_condition(email, condition) for condition in conditions)
        elif predicate == "any":
            return any(self._evaluate_condition(email, condition) for condition in conditions)
        else:
    ...
    def _get_field_value(self, email: Email, field: str) -> Optional[Union[str, datetime]]:
        """
